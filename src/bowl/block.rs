
use dioxus::prelude::*;
use super::*;
use sha2::{Sha256, Digest};
use log::info;

pub enum BlockBoxerEvent {
    Skip,
    WUD,
}

#[inline_props]
pub fn Block<'a>(cx: Scope<'a>, comp_id: ComponentId, on_bonk: EventHandler<'a, BlockBoxerEvent>, debug_info: Option<String>) -> Element<'a> {

    let show_debug_info = true;
    let comp = CONSTELLATION.get_comp(*comp_id);

    

    // BACKGROUND COLOR
    let color_str = {
        match comp.info.owner.value().light_back {
            Some(c) => {c.to_string()},
            None => { // gen random color seeded with component owner name
                let mut hasher = Sha256::new();
                hasher.update(comp.info.owner.to_string());
                let result = hasher.finalize();
                let color_nums = &result[1..=3].to_vec();
                format!("rgba({}, {}, {}, 0.2)", color_nums[0], color_nums[1], color_nums[2])
            }
        }
    };


    let font_col = use_state(cx, || "black");

    cx.render(rsx!{
        div {
            onclick: move |_| {
                if font_col.get() == &"black" {
                    font_col.set("red");
                } else {
                    font_col.set("black");
                }
            },
            class: "block_box normal",
            style: "
                display: flex;
                justify-content: center;
                align-items: center;
                position: relative;
                height: 100%;
                width: 100%;
                background: {color_str};
                font-size: 10px;
                color: {font_col};
            ",
            div {
                class: "block_button delete",
                onclick: move |_| {on_bonk.call(BlockBoxerEvent::Skip)},
            },
            div {
                class: "block_button group",
                onclick: move |_| {on_bonk.call(BlockBoxerEvent::WUD);},
            },
            if show_debug_info && debug_info.is_some() {
                rsx!{div {
                    "{CONSTELLATION.get_comp(*comp_id).info.name}\n{debug_info.as_ref().unwrap()}",
                }}
            } else {
                rsx!{div {
                    "{CONSTELLATION.get_comp(*comp_id).info.name}",
                }}
            }
        }
    })
}