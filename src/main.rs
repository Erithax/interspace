...
fn main() {
    ...
    let c = Constellation::generate();


    // serde_cbor::to_writer(std::fs::File::create("./tempy.cbor").unwrap(), &c).unwrap();

    // let c = Constellation::load();
    // println!("{}", c.get_node(&BlockType::Render(Render::Skia)).fulldowntree);
...
fn App(cx: Scope) -> Element {
    ...
    // let c = ron::from_str::<Constellation>(include_str!("../res/state/constellation.ron")).unwrap();
...
pub fn BlockBox<'a>(
    ...
    let color_str = format!("rgba({}, {}, {}, 0.2)", color_nums[0], color_nums[1], color_nums[2]);
    // info!("{color_str}");
